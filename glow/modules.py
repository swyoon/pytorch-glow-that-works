...
class Permute2d(nn.Module):
    def __init__(self, num_channels, shuffle):
        ...
        self.register_buffer('indices', torch.arange(self.num_channels - 1, -1, -1).to(torch.long))
...
class Split2d(nn.Module):
    ...
    def forward(self, input, logdet=0., reverse=False, eps_std=None, z2=None):
        if not reverse:
            ...
            logpz = GaussianDiag.logp(mean, logs, z2)
            return z1, logdet, logpz, z2
        else:
            z1 = input
            mean, logs = self.split2d_prior(z1)
            if z2 is None:
